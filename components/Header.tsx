...
export function Header() {
  ...
  const linkClass = (active: boolean, desktop: boolean) => {
    if (desktop) {
      return `relative z-10 shrink-0 rounded-full px-2.5 py-2 text-sm font-medium transition-colors xl:px-3 ${
        active ? "bg-sand text-ink" : "text-ink-muted hover:bg-sand hover:text-ink"
      }`;
    }
    return `relative z-10 flex min-h-11 items-center rounded-lg px-3 text-base font-medium ${
    ...
    >
      <div className="page-container flex h-[var(--header-height)] max-w-7xl items-center gap-2 py-0 sm:gap-3">
        ...
        <a
          ...
          className="btn-cta relative z-10 hidden min-h-11 shrink-0 !px-4 !py-2 !text-sm md:inline-flex"
        >
          507 130 571
        </a>
        ...
        <button
          type="button"
          className="relative z-10 inline-flex min-h-11 min-w-11 shrink-0 items-center justify-center rounded-lg text-ink lg:hidden"
          aria-expanded={open}
